...
version: 0.2.0
dependencies: A `./my-agent/` folder from /launch-your-agent. Account auth optional (shell env ANTHROPIC_API_KEY, `ant auth login` token, or `my-agent/.env`) — without any, the wrap-up is describe-only.
---
...
- If auth is available (shell env `ANTHROPIC_API_KEY`, the ant CLI's OAuth token, or a key in `my-agent/.env` — same ladder as `../launch-your-agent/references/cma-api.md` §Setup), refresh live state via the API (shapes: `../launch-your-agent/references/cma-api.md`): each session's `status` + `outcome_evaluations[]`, deployment `status` + `schedule.upcoming_runs_at`, latest `usage`. Parse with python (`strict=False`), never jq.
- No auth at all → continue describe-only and say in one sentence that live status needs auth (`ant auth login` or a key in `.env`).
...
5. **Hygiene sweep — quiet by default.** Do the sweep (archive finished sessions; check the deployment's `initial_events` for literal dates; check no key ever touched the chat and any pasted key lives only in `.env`; stop a still-running `viewer.py` background server — `pkill -f 'python3 viewer.py'` — it proxies live credentials and shouldn't outlive the session; save a passed run as `evals/case-01/expected.md` if there's no golden case yet) and **fix silently what you can**. Only mention something if it's materially relevant — i.e. the founder must act (a key that touched chat → rotate; a literal date you can't patch without their say-so) or it changes how they use the agent. No "✅ all good" lists.
