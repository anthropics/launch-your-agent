...
| `viewer.py` | The run viewer — localhost debug view (animated flow replay of the event log, raw log, rendered report, Run-now button). Copied from `references/run-viewer-template.py` with only the CONFIG block edited (deliverable + render mode + hero tools from the build sheet). Default watch surface at launch; binds 127.0.0.1; same auth ladder as the scripts. |
...
2. **Stage** → validate payloads, write the resumable launch sequence and LAUNCH.md, `ant`/curl check — all without the key. The brief already told them what auth to line up and where (`ant auth login` or an API key; any connector token/webhook), so by now they're set up in parallel. Then the auth step, made as easy as possible: check the shell env first (skip the ask if `ANTHROPIC_API_KEY` is already set); otherwise offer `ant auth login` (browser OAuth, no key handling — see cma-api.md §Setup); last resort, pre-create `my-agent/.env` (chmod 600) and have the founder paste the key into it via its absolute path — never into the chat. Keep staging/explaining while they do. Model pick happens at launch (newest Opus-class; show the full slug from then on).
