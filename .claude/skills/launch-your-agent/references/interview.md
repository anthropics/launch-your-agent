...
| `agent-overview.html` + `overview.css` | The "spec sheet" page: a static view of every primitive in use (agent, rubric, tools, connections, memory, schedule, evals, next directions). **Claude Code writes it once and edits it in place** — copy `references/overview.css` alongside it once, Write the HTML once after the interview, then Edit only the slots that change (status pill, IDs, run-log rows, verdicts) after launch and each iteration; not a script the founder runs. Template/example: `references/overview-template.html`. Live observability stays in the Console. |
...
4. **While it runs** → Edit agent-overview.html in place (status pill → live, drop in IDs as Console links, append run-log row); draft NEXT-DIRECTIONS; seed memory store if planned.
